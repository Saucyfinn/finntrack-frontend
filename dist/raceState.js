import { kmeansTwo, clusterPoints, convexHull } from "./utils";
...
export class RaceState {
    ...
    async fetch(request) {
        ...
        if (request.method === "GET" && path === "/replay-multi")
            return this.json(await this.replayMulti());
        if (request.method === "GET" && path === "/autocourse")
            return this.json(await this.autoDetectCourse());
        if (request.method === "GET" && path === "/export/gpx")
            return this.exportGPX();
        if (request.method === "GET" && path === "/export/kml")
            return this.exportKML();
        if (request.method === "POST" && path === "/archive/save")
            return this.saveArchive();
        if (request.method === "GET" && path === "/archive/load")
            return this.loadArchive(request);
    ...
    async ensureRaceId(raceId) {
        if (this.raceId)
            return;
        this.raceId = raceId;
        await this.state.storage.put("raceId", raceId);
    }
    async loadRaceIdIfNeeded() {
        if (this.raceId)
            return;
        const stored = (await this.state.storage.get("raceId")) || null;
        this.raceId = stored;
    }
    async getBoatIds() {
        return (await this.state.storage.get("boatIds")) || [];
    }
    async addBoatId(boatId) {
    ...
    async hydrateBoatsFromStorageIfEmpty() {
        if (this.boats.size > 0)
            return;
        await this.loadRaceIdIfNeeded();
        const ids = await this.getBoatIds();
        for (const boatId of ids) {
            const last = await this.state.storage.get(`boat:${boatId}:latest`);
            if (last)
                this.boats.set(boatId, last);
        }
    }
    /**
     ...
     */
    async getBoatsSnapshot(activeSeconds = 300) {
    ...
    async handleUpdate(request) {
        ...
        const frame = { raceId, boatId, lat, lng, speed, heading, timestamp };
    ...
    detectStartTime(boatsData) {
        const moves = [];
        for (const frames of Object.values(boatsData)) {
            for (let i = 1; i < frames.length; i++) {
                if ((frames[i].speed ?? 0) > 2.5) {
                    moves.push(frames[i].timestamp);
                    break;
                }
            }
        }
    ...
    detectStartLine(boatsData, startTime) {
        const points = [];
        for (const frames of Object.values(boatsData)) {
    ...
    detectMarks(boatsData) {
        const turnPoints = [];
        for (const frames of Object.values(boatsData)) {
            for (let i = 2; i < frames.length; i++) {
                const h1 = frames[i - 1].heading ?? 0;
                const h2 = frames[i].heading ?? 0;
                if (Math.abs(h1 - h2) > 60)
                    turnPoints.push([frames[i].lat, frames[i].lng]);
            }
        }
        return clusterPoints(turnPoints, 40);
    }
    detectFinishLine(boatsData) {
        const points = [];
        for (const frames of Object.values(boatsData)) {
            if (frames.length > 0) {
    ...
    computeCoursePolygon(boatsData) {
        const pts = [];
        for (const frames of Object.values(boatsData))
            for (const f of frames)
    ...
    async saveArchive() {
        await this.loadRaceIdIfNeeded();
        if (!this.raceId)
            return new Response("No raceId yet", { status: 400 });
        const multi = await this.replayMulti();
    ...
    async loadArchive(request) {
        ...
        if (!rid)
            return new Response("No raceId", { status: 400 });
        const metaObj = await this.env.RACES.get(`races/${rid}/meta.json`);
        if (!metaObj)
            return new Response("No archive found", { status: 404 });
